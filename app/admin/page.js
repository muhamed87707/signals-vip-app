...
export default function AdminPage() {
    ...
    // Auto-Bold Function: Wraps each line in *...* for Telegram Markdown
    const formatTextToBold = (text) => {
        if (!text) return '';
        return text.split('\n').map(line => line.trim() ? `*${line.trim()}*` : '').join('\n');
    };

    // Step 1: Handle File Selection & Generate Preview
    const handlePreview = async (e) => {
        const file = e.target.files?.[0];
        if (!file) return;

        setUploading(true); // Temporary loading state for processing
        setError('');
        setSuccessMessage('');
        ...
        try {
            ...
            reader.onloadend = async () => {
                ...
                // Auto-Bold the text
                postToUse = formatTextToBold(postToUse);

                // Generate blurred image if needed for preview
        ...
        try {
            const res = await fetch('/api/signals', {
                ...
                body: JSON.stringify({
                    pair: 'GOLD',
                    type: 'SIGNAL',
                    imageUrl: previewData.base64Image,
                    telegramImage: previewData.telegramImage,
                    sendToTelegram: previewData.postToTelegram,
                    isVip: previewData.signalType === 'vip',
                    customPost: previewData.postToUse || null
                })
            });
            ...
            if (data.success) {
                ...
                if (previewData.postToTelegram) msg += ` ${t.telegramSuccess || ''}`;
                ...
                setCustomPost(''); // Optional: Clear post after success
            } else {
    ...
    // Replace the Upload Button Section with this:
    {/* ===== Upload / Preview Section ===== */ }
    {
        !previewData ? (
            <div style={{ textAlign: 'center' }}>
                <input ref={fileInputRef} type="file" accept="image/*" onChange={handlePreview} style={{ display: 'none' }} id="image-upload" />
        ...
        <div style={{
            ...
            <h3 style={{ color: '#DAA520', marginBottom: '1.5rem', fontSize: '1.5rem' }}>
                👀 {lang === 'ar' ? 'معاينة النشر' : 'Publish Preview'}
            </h3>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '2rem', justifyContent: 'center', marginBottom: '2rem' }}>
                ...
                <div style={{ flex: '1 1 300px', maxWidth: '400px' }}>
                    <div style={{ color: '#aaa', marginBottom: '0.5rem', fontSize: '0.9rem' }}>
                        {lang === 'ar' ? 'الصورة العامة (مجانية)' : 'Public Image (Free)'}
                    </div>
                    <img src={previewData.base64Image} alt="Preview" style={{ width: '100%', borderRadius: '12px', border: '1px solid #333' }} />
                </div>

                {/* Telegram Image Preview (if VIP) */}
                {previewData.telegramImage && (
                    <div style={{ flex: '1 1 300px', maxWidth: '400px' }}>
                        <div style={{ color: '#aaa', marginBottom: '0.5rem', fontSize: '0.9rem' }}>
                            {lang === 'ar' ? 'صورة تليجرام (مموهة)' : 'Telegram Image (Blurred)'}
                        </div>
                        <img src={previewData.telegramImage} alt="Blurred Preview" style={{ width: '100%', borderRadius: '12px', border: '1px solid #333' }} />
                    </div>
                )}
            </div>

            {/* Validated Text Preview */}
            <div style={{ marginBottom: '2rem', textAlign: 'left', background: '#000', padding: '1.5rem', borderRadius: '12px', border: '1px solid #333' }}>
                <div style={{ color: '#aaa', marginBottom: '0.5rem', fontSize: '0.8rem', textTransform: 'uppercase' }}>
                    {lang === 'ar' ? 'النص النهائي (BOLD)' : 'Final Text (Bold)'}
                </div>
                <pre style={{
                    whiteSpace: 'pre-wrap',
                    color: '#fff',
            ...
            <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center' }}>
                ...
                >
                    {uploading ? t.uploading : (lang === 'ar' ? '✅ تأكيد ونشر الآن' : '✅ Confirm & Publish')}
                </button>
            </div>
        </div>
    )
